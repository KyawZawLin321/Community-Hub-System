...
$(document).ready(function() {
    ...
    $('#exampleModalPoll1').on('show.bs.modal', function (event) {
        ...
        // Set the end date
        $('#endDate1').val(formatDate1(endDate)); // Ensure the date is in the correct format
        ...
        if (typeof optionTexts === 'string') {
            ...
            optionTexts.forEach((optionText, index) => {
                const optionInput = `<div class="mb-3 option-item">
                                        <input type="text" class="form-control option-input2" value="${optionText}" placeholder="Option ${index + 1}" style="font-family: Math">
                                        <button type="button" class="btn btn-danger btn-remove-option" style="margin-top: -37px;margin-left: 522px" aria-label="Remove option">&times;</button>
                                    </div>`;
                $('#optionsContainer9').append(optionInput);
            });
        } else {
...
document.getElementById('submitBtnPoll1').addEventListener('click', async () => {
    ...
    const optionInputs = document.querySelectorAll('.option-input2');
    const optionTexts = Array.from(optionInputs).map(input => input.value);
...
//    try {
//        const response = await fetch('/userPoll');
