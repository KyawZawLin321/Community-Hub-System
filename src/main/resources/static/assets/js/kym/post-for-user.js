...
const fetchData = async () => {
    ...
    const contentResponse = await fetch('/userContents');
    const shareResponse = await fetch('/userShares');
    const pollResponse = await fetch('/userPoll');
...
// Check if there are any images or videos
    ...
    if (hasImages || hasVideos) {
        ...
        rowData.imageUrls.forEach((imageUrl, imgIndex) => {
            ...
            image.src = imageUrl;
            image.classList.add('d-block', 'w-100');
            image.style.height = '340px'; // Set a fixed height for images
            image.alt = `Slide ${imgIndex + 1}`;
