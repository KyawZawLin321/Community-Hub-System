...
const fetchData = async () => {
        const contentResponse = await fetch('/contents');
        const shareResponse = await fetch('/shares');
        const pollResponse = await fetch('/polls');
        ...
        const pollList=await pollResponse.json();
        // Merge the lists
...
// Create the copyable link item in the dropdown menu
//                const copyLinkItem = document.createElement('li');
//                copyLinkItem.innerHTML = `
//    <button class="dropdown-item copy-link" type="button" data-content-id="${rowData.id}">
//        Copy Link
//    </button>
//`;

// Append the copyable link item to the dropdown menu
  //              dropdownMenu.appendChild(copyLinkItem);

// Event listener for copying the link
//                copyLinkItem.querySelector('.copy-link').addEventListener('click', function() {
//                    const contentId = this.getAttribute('data-content-id');
//                    // const contentUrl = `${window.location.origin}/user/searchcontent?contentId=${contentId}`;
//                    const contentUrl = `https://yourapp.com/user/searchcontent?contentId=${rowData.id}`;
//                    const dummyInput = document.createElement('input');
//                    document.body.appendChild(dummyInput);
//                    dummyInput.setAttribute('value', contentUrl);
//                    dummyInput.select();
//                    document.execCommand('copy');
//                    document.body.removeChild(dummyInput);
//                    alert('Link copied to clipboard: ' + contentUrl);
//
//                    // Call the search function with the copied link
...
// Check if there are any images or videos
    ...
    if (hasImages || hasVideos) {
        ...
        rowData.imageUrls.forEach((imageUrl, imgIndex) => {
            ...
            image.style.height = '350px'; // Set a fixed height for images
            ...
            image.addEventListener('click', () => {
                ...
                downloadImageBtn.onclick = async () => {
                    try {
                        ...
                        link.download = `image_${imgIndex + 1}.jpg`; // Set the desired filename
...
// Append the carousel to the document body or a specific container
    ...
    // Create the post bottom section
        ...
        heartIcon.id = `like-post-${rowData.id}`;
    ...
    console.log('like count'+rowData.likeCount)
    //swm
...
// i change place of swm
    ...
    if ((rowData.imageUrls && rowData.imageUrls.length) + (rowData.videoUrls && rowData.videoUrls.length) > 1) {
     ...
     //   post.appendChild(editButton);
        ...
        const postDiv = document.querySelector('.appendContent');
...
const text1 = document.getElementById("text1");
   ...
   function validateForm1() {
       const textValid = text1.value.trim() !== "";
       const photoValid = photo.children.length > 0;
       const videoValid = video.children.length > 0;
       const filesValid = fileInput.files.length > 0;
       submitBtn.disabled = !(textValid || photoValid || videoValid || filesValid);


   }

   // Add event listeners to the input fields to dynamically update the state of the submit button
   text1.addEventListener("input", validateForm1);
   photo.addEventListener("change",validateForm1);
   video.addEventListener("change",validateForm1);
    fileInput.addEventListener("change",validateForm1);

   // Call the function initially to set the initial state of the submit button
   validateForm1();

// to show data in update post12 modal form //
function clearChoose1() {
    $(".uploaded-photos").empty(); // Clear uploaded photos container
    $('#multipartFile1').val(''); // Reset file input field
}
...
const populatedTable1 = async (share) => {
    try {
        ...
        if (!response11.ok) {
            throw new Error('Failed to fetch shared user data');
        }
        const userData1 = await response11.json();
        console.log("userData1 :" + userData1.id);
        // Fetch content data
        ...
        if (loginUserId == userData1.id) {
            ...
            // Create the delete item in the dropdown menu
            const deleteItem = document.createElement('li');
            deleteItem.innerHTML = `<button class="dropdown-item" type="button" data-bs-toggle="modal" data-bs-target="#deleteShareModal" data-share-id=${share.id}>Delete</button>`;
        ...
        ownerPhoto.style.cursor = 'pointer';
...
// Check if content data is available
        if (contentData) {
            ...
            textP.id = 'text3';
            textP.style.marginTop = '20px';
            textP.style.marginBottom = contentData.text.length > 130 ? '30px' : '33px';
            textP.style.marginRight = '33px';

// Function to style mentions
            function styleMentions(text) {
                return text.replace(/@([\w\s]+?)(?=\s|$|[^@\w\s])/g, mention => {
                    return `<span class="mention">${mention}</span>`;
                });
            }

// Initially truncated content with mentions
            const truncatedText = contentData.text.substring(0, 130) + '...';
            textP.innerHTML = styleMentions(contentData.text.length > 130 ? truncatedText : contentData.text);

            if (contentData.text.length > 130) {
                const seeMoreButton = document.createElement('button');
                seeMoreButton.textContent = 'See More';

                seeMoreButton.addEventListener('click', function () {
                    if (seeMoreButton.textContent === 'See More') {
                        textP.innerHTML = styleMentions(contentData.text);
                        seeMoreButton.textContent = 'See Less';
                    } else {
                        textP.innerHTML = styleMentions(truncatedText);
                        seeMoreButton.textContent = 'See More';
                    }

                    // Re-append the button after toggling text
                    textP.appendChild(seeMoreButton);
                });
            ...
            if (hasImages || hasVideos) {
                ...
                // if (contentData.imageUrls.length > 1 || (contentData.videoUrls && contentData.videoUrls.length > 1)) {
                if ((contentData.imageUrls && contentData.imageUrls.length) + (contentData.videoUrls && contentData.videoUrls.length) > 1) {
...
//             $('#userId').val(userId);
//             $('#exampleModal1').modal('show');
//             var captionTextarea = document.getElementById("caption");
//             captionTextarea.value="";
//             document.getElementById("error-share").style.display="none";
//
//
...
// Event listener to clear data when modal is closed
        ...
        const formPost = document.querySelector('.appendContent');
...
$(document).ready(function() {
    ...
    document.getElementById("error-shareUpdate").style.display="none";
    var caption4 = $("#caption4");
    const submitBtn3 = document.getElementById('submitBtn3');

    // Disable the submit button initially
    caption4.on('input', shareUpdateForm);

    // Function to validate the form
    function shareUpdateForm() {
        var captionValue4 = caption4.val().trim(); // Get the trimmed value of the textarea
            // Show modal message
        if(captionValue4.length > 300 ){
                document.getElementById("error-shareUpdate").textContent = "You can type a maximum of 300 characters.";
                document.getElementById("error-shareUpdate").style.display="block";
                submitBtn3.disabled = true; // Enable the button if either textarea or file input is not empty
                return;
        }
        else{
                    document.getElementById("error-shareUpdate").style.display="none";
                submitBtn3.disabled = false; // Enable the button if either textarea or file input is not empty
