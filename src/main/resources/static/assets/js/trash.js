...
const fetchData = async () => {
    ...
    const contentResponse = await fetch('/trash-content');
    const shareResponse = await fetch('/trash-share');
...
// Create the dropdown menu
  ...
  <button type="button" class="dropdown-item" data-bs-toggle="modal" data-bs-target="#recycleModal" data-content-id=${rowData.id}>
    restore
</button>`;
...
const restoreContent = async (id) => {
    const url = `/restore-content/${id}`;
    const restoreContent = await fetch(url,{
        method:'PUT',
    });
    if(restoreContent.ok){
         clearPreviousPosts();
                await fetchData();
